...
namespace BruteForceMaster
{
    class PasswordFileHandler
    {
        ...
        public static void WriteClearPasswordFile(String filename, String[] usernames, String[] passwords)
        {
            if (usernames.Length != passwords.Length)
            {
                throw new ArgumentException("usernames and passwords must be same lengths");
            ...
            using (StreamWriter sw = new StreamWriter(fs))
            {
                for (int i = 0; i < usernames.Length; i++)
                {
                    String line = usernames[i] + ":" + passwords[i] + "\n";
                    sw.WriteLine(line);
                }
            }
        }
        ...
        public static void SendPasswordFile(string filename, NetworkStream networkStream)
        {
            FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read);
            using (StreamReader sr = new StreamReader(fs))
            {
        ...
        public static void StampPasswordFile(string filename)
        {
            using (FileStream fs = new FileStream(filename, FileMode.Append, FileAccess.Write))
            using (StreamWriter sw = new StreamWriter(fs))
            {
                sw.WriteLine("Cracking finished at:" + DateTime.UtcNow);
                sw.WriteLine();
            }
        }
    }
